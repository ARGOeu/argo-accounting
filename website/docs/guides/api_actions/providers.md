...
**2.** [Contact](mailto:argo@grnet.gr) the system administrator to assign you one or more [roles](/docs/authorization/accounting_system_roles.md) on the Provider that is associated with the particular Project.

### NOTE

In the Accounting Service, the `Provider Admin` role is the main role for
...
> 📝 **For more details on how to syntax the request, see** [here](../../api/provider#get---fetch-an-existing-provider).
...
[here](../../api/installation#post---create-a-new-installation).
...
> 📝 **For more details on how to syntax the request, see**
[here](../../api/installation#get-fetch-all-provider-installations).
...
on each field of the **[Installation Collection](../../api/installation)**
...
> 📝 **For more details on how to structure the request, see** [here](../../api/provider#get---get-provider-report).


### GET Global Provider Report

You can get a global report of a Provider by applying a request to the Accounting
Service API. The report contains aggregated metric values for all Installations for a specific time period, independently of a Project. The report contains aggregated metric values grouped by metric definition for a specific time period. When capacities are defined for the selected time period, the report also includes usage percentages broken down by the capacity-defined time periods.

> 📝 **For more details on how to structure the request, see** [here](../../api/provider#get---get-global-provider-report).
